...
/// Get Budgets query.
/// </summary>
public record GetTransaction(Guid BudgetId) : IQuery<PagedList<TransactionInfo>>;
...
public class GetTransactionQueryHandler : IQueryHandler<GetTransaction, PagedList<TransactionInfo>>
{
	...
	{
		var transcations = await this.budgetDbContext.Transaction.Where(x => x.BudgetId.Value == query.BudgetId).ToListAsync();
