...
public class EditBudgetValidator : AbstractValidator<EditBudget>
{
	...
	public EditBudgetValidator(BudgetDbContext budgetDbContext)
	{
		...
		this.RuleFor(budget => new { budget.Id, budget.Name }).MustAsync(async (x, cancellation) => await this.IsNameUniqueWithinUserBudgets(x.Id, x.Name, cancellation))
			.WithMessage("Name already exists in your budgets. Choose a different name.").WithErrorCode("1.4");
		this.RuleFor(budget => new { budget.Period.StartDate, budget.Period.EndDate }).Must(x => x.StartDate <= x.EndDate)
