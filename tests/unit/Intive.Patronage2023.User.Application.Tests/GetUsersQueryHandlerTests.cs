...
/// <summary>
/// Test that checks the behavior of a query instance class "GetBudgetDetailsQueryHandlerTests".
/// </summary>
public class GetUsersQueryHandlerTests
{
	private readonly Mock<IKeycloakService> keycloakServiceMock;
	private readonly GetUsersQueryHandler instance;
	...
	public GetUsersQueryHandlerTests()
	{
		...
		this.instance = new GetUsersQueryHandler(this.keycloakServiceMock.Object);
	}
	...
	public async Task Handle_WhenCalled_ShouldReturnListOfUsers()
	{
		...
		// Act
		var response = await this.instance.Handle(query, CancellationToken.None);
	...
	public async Task Handle_WhenCalledWithInvalidSortParameters_ShouldThrowException()
	{
		...
		// Act and Assert
		await Assert.ThrowsAsync<NotSupportedException>(async () => await this.instance.Handle(query, CancellationToken.None));
	}
